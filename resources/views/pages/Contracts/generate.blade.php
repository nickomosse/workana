...
@section('content')
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{route('contracts.store')}}" class="form" method="POST">
                @csrf
                <div class="row">
                    <div class="form-group col-md-4">
                        <label>Nome:</label>
                        <input type="text" name="name" class="form-control" placeholder="Artur Santos" value="{{$estimate->name}}" required>
                    </div>
                    <div class="form-group col-md-4">
                        <label>Nascimento:</label>
                        <input type="date" name="birthday" class="form-control" id="birthday" required value="{{$estimate->birthday}}">
                    </div>
                    <div class="form-group col-md-4">
                        <label for="gender">Gênero:</label>
                        <select class="custom-select rounded-0" id="gender" name="gender" required>
                            <option @if ($estimate->gender == 1) selected @endif value=1>Masculino</option>
                            <option @if ($estimate->gender == 2) selected @endif value=0>Feminino</option>
                        </select>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-md-3">
                        ...
                        <input type="date" name="date" id="date" class="form-control" placeholder="06/04/2025" value="{{$estimate->date}}" required>
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <select class="custom-select rounded-0" id="package" name="package" required>
                            ...
                            @foreach ($packages as $package)
                                <option @if ($estimate->package->id == $package->id) selected @endif value="{{$package->id}}" data-info="{{$package->price}}">{{$package->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group col-md-3">
                        ...
                        <select class="custom-select rounded-0" id="referral" name="referral" required>
                            @foreach ($referrals as $referral)
                                <option @if ($estimate->referral->id == $referral->id) selected @endif value="{{$referral->id}}">{{$referral->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group col-md-3">
                        ...
                        <select class="custom-select rounded-0" id="partyTime" name="partyTime_id" required>
                            @foreach ($partyTimes as $partyTime)
                                <option @if ($estimate->inittime == $partyTime->inittime && $estimate->endtime == $partyTime->endtime) selected @endif value="{{$partyTime->id}}">{{date('H:i', strtotime($partyTime->start))}} às {{date('H:i', strtotime($partyTime->end))}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <input type="text" name="total" class="form-control" id="total" value="{{$estimate->total}}">
                    </div>
                    <div class="form-group col-md-3">
                        <label for="signal">Sinal:</label>
                        <input type="text" name="signal" class="form-control" id="signal" required value="{{$estimate->signal}}">
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <input type="text" name="installments" class="form-control" id="installments" value="1.000" required value="{{$estimate->installments}}">
                    </div>
                ...
                <div class="row">
                    <div class="form-group col-md-4">
                        ...
                        <input type="text" name="frespname" class="form-control" placeholder="Tereza Cruz" required value="{{$estimate->frespname}}">
                    </div>
                    <div class="form-group col-md-4">
                        <label>Email do responsável 1:</label>
                        <input type="email" name="frespemail" class="form-control" placeholder="tere@hotmail.com" required value="{{$estimate->frespemail}}">
                    </div>
                    <div class="form-group col-md-4">
                        <label>Celular do responsável 1:</label>
                        <input type="text" name="frespcel" class="form-control" placeholder="284453448" required value="{{$estimate->frespcel}}">
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="form-group col-md-4">
                        ...
                        <input type="text" name="srespname" class="form-control" placeholder="Marcos Cruz" required value="{{$estimate->srespname}}">
                    </div>
                    <div class="form-group col-md-4">
                        <label>Celular do responsável 2:</label>
                        <input type="text" name="srespcel" class="form-control" placeholder="21985641245" required value="{{$estimate->srespcel}}">
                    </div>
                    <div class="form-group col-md-4">
                        <label>Número do orçamento:</label>
                        <input type="text" name="estimate" class="form-control" required value="{{$estimate->id}}">
                    </div>
                </div>
                <hr>
                ...
                <div class="row">
                    <div class="col-sm-4">
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="icomment" rows="5">Comentário padrão</textarea>
                    ...
                    <div class="col-sm-4">
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="ecomment" rows="5">Comentário padrão</textarea>
                        ...
                        <div class="form-group">
                            ...
                            @foreach ($observations as $observation)
                                <div class="custom-control custom-checkbox">
                                    <input class="custom-control-input custom-control-input-danger" type="checkbox" id="{{$observation->id}}" name="observations[]" value="{{$observation->id}}" @if          ($estimate->observations->contains($observation))
                                    checked
                                    @endif>
